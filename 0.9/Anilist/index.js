  ...
  var AniListExtension = class {
    ...
    async getItems(query, queryVariables, metadata, search) {
      ...
      for (const searchResult of searchResults) {
        let title = searchResult.title.userPreferred ?? searchResult.title.english ?? searchResult.title.native ?? "No Title";
        let relevance = 0;
        if (typeof search == "string") {
          relevance = relevanceScore(title, search);
        }
        result.push({
          manga: {
    ...
    async processChapterReadActionQueue(chapterReadActions) {
      ...
      for (const readAction of chapterReadActions) {
        try {
          let anilistManga = anilistMangaCache[readAction.sourceManga.mangaId];
          if (!anilistManga) {
            const variables = {
