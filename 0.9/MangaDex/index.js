...
var source = (() => {
              ...
              getSections() {
                return [
                  (0, import_types.Section)("proxySection", [
                    (0, import_types.LabelRow)("proxyInfo", {
  ...
  async function trimWhitespace(imageArrayBuffer) {
    ...
    if (right < left || bottom < top) {
      throw new Error("Image is blank or contains only whitespace");
    }
    const trimmedWidth = right - left + 1;
    const trimmedHeight = bottom - top + 1;
    const trimmedImg = make(trimmedWidth, trimmedHeight);
    const trimmedCtx = trimmedImg.getContext("2d");
  ...
  function encodePNGToArrayBuffer(img) {
    return new Promise((resolve, reject) => {
      ...
      const stream = {
        ...
        on: (event, callback) => {
          if (event === "error") {
            reject(new Error("Error encoding image"));
          }
        }
      };
