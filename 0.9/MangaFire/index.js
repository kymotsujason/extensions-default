...
var source = (() => {
  ...
  // src/MangaFire/MangaFireHelper.ts
  init_buffer();
  var MFGenresClass = class {
    constructor() {
      this.Genres = [
        { param: "1", name: "Action" },
        { param: "78", name: "Adventure" },
        { param: "3", name: "Avant Garde" },
        { param: "4", name: "Boys Love" },
        { param: "5", name: "Comedy" },
        { param: "77", name: "Demons" },
        { param: "6", name: "Drama" },
        { param: "7", name: "Ecchi" },
        { param: "79", name: "Fantasy" },
        { param: "9", name: "Girls Love" },
        { param: "10", name: "Gourmet" },
        { param: "11", name: "Harem" },
        { param: "530", name: "Horror" },
        { param: "13", name: "Isekai" },
        { param: "531", name: "Iyashikei" },
        { param: "15", name: "Josei" },
        { param: "532", name: "Kids" },
        { param: "539", name: "Magic" },
        { param: "533", name: "Mahou Shoujo" },
        { param: "534", name: "Martial Arts" },
        { param: "19", name: "Mecha" },
        { param: "535", name: "Military" },
        { param: "21", name: "Music" },
        { param: "22", name: "Mystery" },
        { param: "23", name: "Parody" },
        { param: "536", name: "Psychological" },
        { param: "25", name: "Reverse Harem" },
        { param: "26", name: "Romance" },
        { param: "73", name: "School" },
        { param: "28", name: "Sci-Fi" },
        { param: "537", name: "Seinen" },
        { param: "30", name: "Shoujo" },
        { param: "31", name: "Shounen" },
        { param: "538", name: "Slice of Life" },
        { param: "33", name: "Space" },
        { param: "34", name: "Sports" },
        { param: "75", name: "Super Power" },
        { param: "76", name: "Supernatural" },
        { param: "37", name: "Suspense" },
        { param: "38", name: "Thriller" },
        { param: "39", name: "Vampire" }
      ];
  ...
  var MFGenres = new MFGenresClass();
  var MFLanguagesClass = class {
    ...
    getMFCodeList() {
      return this.Languages.map((Language) => Language.MFCode);
    }
    getName(MFCode) {
      return this.Languages.filter((Language) => Language.MFCode == MFCode)[0]?.name ?? "Unknown";
    }
    getLangCode(MFCode) {
      return this.Languages.filter((Language) => Language.MFCode == MFCode)[0]?.lang ?? "Unknown";
    }
    getDefault() {
      return this.Languages.filter((Language) => Language.default).map(
        (Language) => Language.MFCode
      );
    }
  };
  var MFLanguages = new MFLanguagesClass();

  // src/MangaFire/MangaFireParser.ts
  init_buffer();
  var import_types2 = __toESM(require_lib());

  // src/MangaFire/RelevanceScore.ts
  init_buffer();
  ...
  var parseMangaDetails = ($2, mangaId, url) => {
    ...
    const rawStatus = $2("div.info > p").text().trim();
    let status = "ONGOING";
  ...
  var parseChapterList = ($2, sourceManga) => {
    ...
    const chapters = [];
    let sortingIndex = 0;
    for (const chapter of $2("div.list-body > ul > li").toArray()) {
      const title = $2("span", chapter).first().text().trim();
      const chapterId = $2("a", chapter).attr("href")?.replace(/\/$/, "")?.split("/").pop() ?? "";
      const groupElement = $2('div.attr-item span:contains("Magazines")').next(
        "span"
      );
      const group = groupElement.length ? groupElement.children().map((_, e) => {
        return $2(e).text().trim();
      }).toArray().join(", ") : "";
      if (!chapterId) continue;
      let language = MFLanguages.getLangCode($2("em").attr("data-lang") ?? "");
      if (language === "Unknown") language = "\u{1F1EC}\u{1F1E7}";
      const timeAgo = $2("span", chapter).last().text().trim().split(" ");
      let date = new Date(Date.now());
      if (timeAgo[1] == "secs")
        date = new Date(Date.now() - 1e3 * Number(timeAgo[0]));
      if (timeAgo[1] == "mins")
        date = new Date(Date.now() - 1e3 * 60 * Number(timeAgo[0]));
      if (timeAgo[1] == "hours")
  ...
  var getImageSrc = (imageObj, source) => {
    let image;
  ...
  var parseChapterDetails = ($2, mangaId, chapterId, url) => {
    const pages = [];
    for (const obj of $2(".page-wrapper").get()) {
      const page = getImageSrc($2(obj), url);
      if (!page) {
        console.log(
          `Could not parse pages for postId:${mangaId} chapterId:${chapterId}`
        );
        continue;
      }
  ...
  var parseViewMore = ($2) => {
    ...
    for (const obj of $2(".item", "#series-list").toArray()) {
      ...
      const mfcode = $2("em", obj).attr("data-lang");
  ...
  var parseTags = () => {
    const arrayTags = [];
    for (const label of MFGenres.getGenresList()) {
      const id = encodeURI(MFGenres.getParam(label) ?? label);
      if (!id || !label) continue;
      arrayTags.push({ id, title: label });
    }
  ...
  var parseSearch = ($2, langFilter, langs, query) => {
    const mangas = [];
    for (const obj of $2("div.original.card-lg > div.unit").toArray()) {
      const id = $2(".poster", obj).attr("href")?.replace("/manga/", "")?.trim().split("/")[0] ?? "";
      const title = $2(".info", obj).text() ?? "";
      const mfcode = $2("em", obj).attr("data-lang");
      const lang = mfcode ? MFLanguages.getLangCode(mfcode) : "\u{1F1EC}\u{1F1E7}";
      const subtitle = lang + " " + $2("ul:nth-child(3) > li:nth-child(1) > a > span:nth-child(1)", obj).text().split("<b>")[0].trim();
      const image = $2("img", obj).attr("src") ?? "";
  ...
  function getLanguageHomeFilter() {
    return Application.getState("language_home_filter") ?? false;
  }
  function getLanguageSearchFilter() {
    return Application.getState("language_search_filter") ?? false;
  }
  ...
  function setLanguageHomeFilter(value) {
    Application.setState(value, "language_home_filter");
  }
  function setLanguageSearchFilter(value) {
  ...
  var BatotoSettingsForm = class extends import_types3.Form {
    getSections() {
      const languages = getLanguages();
      return [
        (0, import_types3.Section)("proxy", [
          (0, import_types3.NavigationRow)("proxy", {
            title: "Proxy Settings",
            form: new class extends import_types3.Form {
              getSections() {
                return [
                  (0, import_types3.Section)("proxySection", [
                    (0, import_types3.LabelRow)("proxyInfo", {
                      title: "Prehash the password with an online bcrypter"
                    }),
                    (0, import_types3.InputRow)("proxyinput", {
                      title: "Proxy Server",
                    ...
                    (0, import_types3.InputRow)("userinput", {
                    ...
                    (0, import_types3.InputRow)("passinput", {
                    ...
                    (0, import_types3.ToggleRow)("enableProxy", {
                    ...
                    (0, import_types3.ButtonRow)("testProxy", {
                      title: "Test Proxy",
                    ...
                    (0, import_types3.ButtonRow)("login", {
        ...
        (0, import_types3.Section)("contentSettings", [
          ...
          (0, import_types3.ToggleRow)("language_home_filter", {
          ...
          (0, import_types3.ToggleRow)("language_search_filter", {
    ...
    async filterHomeLanguageDidChange(value) {
      setLanguageHomeFilter(value);
    }
    async filterSearchLanguageDidChange(value) {
      setLanguageSearchFilter(value);
    }
  };

  // src/MangaFire/main.ts
  var MANGAFIRE_DOMAIN = "https://mangafire.to";
  var MangaFireInterceptor = class extends import_types4.PaperbackInterceptor {
    async interceptRequest(request) {
      request.headers = {
        ...request.headers ?? {},
        ...{
          referer: `${MANGAFIRE_DOMAIN}/`,
  ...
  var MangaFireExtension = class {
    constructor() {
      ...
      this.mainRequestInterceptor = new MangaFireInterceptor("main");
      this.cookieStorageInterceptor = new import_types4.CookieStorageInterceptor({
    ...
    async getMangaDetails(mangaId) {
      const url = `${MANGAFIRE_DOMAIN}/manga/${mangaId}`;
      const request = {
        url,
        method: "GET"
      };
    ...
    async getChapters(sourceManga) {
      const request = {
        url: `${MANGAFIRE_DOMAIN}/manga/${sourceManga.mangaId}`,
        method: "GET"
      };
      const $2 = await this.fetchCheerio(request);
    ...
    async getChapterDetails(chapter) {
      ...
      const url = `${MANGAFIRE_DOMAIN}/read/${mangaId}/${MFLanguages.getDefault()}/${chapterId}`;
      const request = {
        url,
        method: "GET"
      };
      const $2 = await this.fetchCheerio(request);
      let chapters = parseChapterDetails(
        $2,
        mangaId,
        chapterId,
        MANGAFIRE_DOMAIN
      ...
      if (enableProxyServer && proxyURL != "") {
        ...
        const [_, buffer] = await Application.scheduleRequest(request2);
        const data2 = Application.arrayBufferToUTF8String(buffer);
    ...
    async getDiscoverSectionItems(section, metadata) {
      ...
      const request = {
        url: `${MANGAFIRE_DOMAIN}/browse${param}`,
        method: "GET"
      };
      const $2 = await this.fetchCheerio(request);
    ...
    async getSearchResults(query, metadata) {
      ...
      if (query.title) {
        request = {
          url: `${MANGAFIRE_DOMAIN}/filter?keyword=${encodeURI(
            query.title ?? ""
          )}&page=${page}`,
          method: "GET"
        };
      } else {
        let url = `${MANGAFIRE_DOMAIN}/filter?keyword=&genre[]=`;
        let included = "";
        let excluded = "";
      ...
      const langs = getLanguages() ?? MFLanguages.getDefault();
      const $2 = await this.fetchCheerio(request);
      const manga = parseSearch($2, langSearchFilter, langs, query);
      metadata = !isLastPage($2) ? { page: page + 1 } : void 0;
      return {
        items: manga,
        metadata
