...
var source = (() => {
  ...
  // src/Kunmanga/main.ts
  var main_exports = {};
  __export(main_exports, {
    Kunmanga: () => Kunmanga,
    KunmangaExtension: () => KunmangaExtension
  });
  init_buffer();
  ...
  var Parser3 = class {
    ...
    async parseMangaDetails($2, mangaId, source) {
      ...
      const artist = Application.decodeHTMLEntities(
      ...
      for (const obj of $2("li.wp-manga-chapter  ").toArray()) {
        ...
        if (typeof timeSelector !== "undefined") {
          ...
          mangaTime = this.parseDate(
            $2("span.chapter-release-date > i", obj).text().trim()
          );
        }
        if (!mangaTime.getTime()) mangaTime = /* @__PURE__ */ new Date();
        if (!id || typeof id === "undefined" || id === "#") {
          console.log(
            `Could not parse out ID when getting chapters for postId:${mangaId} parsedId: ${id}`
          );
          continue;
        }
        chapters.push({
    ...
    async parseChapterDetails($2, mangaId, chapterId, selector, source) {
      ...
      for (const obj of $2(selector).get()) {
        const page = await this.getImageSrc(
          $2(obj),
          source
        );
        if (!page) {
    ...
    async parseSearchResults($2, source) {
      ...
      for (const obj of $2(source.searchMangaSelector).toArray()) {
        ...
        if (!slug || !path) {
          throw new Error(
            `Unable to parse slug (${slug}) or path (${path})!`
          );
        }
  ...
  // src/Kunmanga/main.ts
  var KUNMANGA_DOMAIN = "https://kunmanga.com";
  var KunmangaInterceptor = class extends import_types3.PaperbackInterceptor {
    async interceptRequest(request) {
      request.headers = {
        ...request.headers ?? {},
        ...{
  ...
  var KunmangaExtension = class {
    constructor() {
      ...
      this.mainRequestInterceptor = new KunmangaInterceptor("main");
    ...
    async getSearchTags() {
      ...
      const $2 = await this.fetchCheerio(request);
      return this.parser.parseTags($2, true);
    }
    ...
    checkCloudflareStatus(status) {
      if (status == 503 || status == 403) {
        throw new import_types3.CloudflareError({ url: KUNMANGA_DOMAIN, method: "GET" });
      }
    }
  ...
  var Kunmanga = new KunmangaExtension();
