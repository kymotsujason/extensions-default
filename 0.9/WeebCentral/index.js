...
Type: ${row["type"]}`
  ...
  var Parser = class {
    constructor() {
      this.parseChapterDetails = ($2, mangaId, chapterId) => {
        const pages = [];
    ...
    parseViewMore($2) {
      const manga = [];
      const collectedIds = [];
      for (const obj of $2("article").toArray()) {
        const image = $2("source", obj).attr("srcset") ?? "";
        const title = $2("img", obj).attr("alt") ?? "";
        const id = $2("a", obj).first().attr("href")?.replace(/\/$/, "")?.split("/").slice(-2)[0] ?? "";
        const getChapter = $2("div.opacity-70", obj).first().text().trim();
  ...
  var WeebCentralExtension = class extends import__2.Source {
    ...
    async getSearchResults(query, metadata) {
      ...
      this.checkResponseError(response);
      const $2 = this.cheerio.load(response.data);
      const results = await this.parser.parseSearchResults($2);
      metadata = this.parser.isLastPage($2) ? void 0 : { offset: offset + LIMIT };
    ...
    async getHomePageSections(sectionCallback) {
      ...
      this.checkResponseError(response);
      const $2 = this.cheerio.load(response.data);
      this.parser.parseHomeSections($2, sectionCallback);
    }
    async getViewMoreItems(homepageSectionId, metadata) {
      ...
      const $2 = this.cheerio.load(response.data);
      const manga = this.parser.parseViewMore($2);
      return App.createPagedResults({
        results: manga,
...
Please go to the homepage of <${this.baseUrl}> and press the cloud icon.`
  ...
  var WeebCentral = (0, import__2.CompatWrapper)(
    ...
    new WeebCentralExtension(browser_exports)
  );
