...
var source = (() => {
  ...
  __export(main_exports, {
    ...
    WeebCentralExtension: () => WeebCentralExtension
  });
  init_buffer();
  ...
  var parseMangaDetails = ($2, mangaId, url) => {
    ...
    const authors = [];
    for (const authorObj of $2('strong:contains("Author")').siblings().toArray()) {
      const author2 = $2("a", authorObj).text().trim();
      authors.push(author2);
    }
    const author = authors.join(", ");
    const parsedStatus = $2('strong:contains("Status")').next().text().trim();
    let status;
    switch (parsedStatus) {
      case "Ongoing":
        status = "Ongoing";
    ...
    const genres = [];
    for (const genreObj of $2(
      "a",
      $2('strong:contains("Tags(s)")').siblings()
    ).toArray()) {
      const genre = $2(genreObj).text().trim();
  ...
  var parseChapterDetails = ($2, mangaId, chapterId) => {
    const pages = [];
    for (const img of $2("img", "section.cursor-pointer").toArray()) {
  ...
  var parseTags = ($2) => {
    ...
    for (const genreObj of $2(
      "span",
      $2("fieldset.collapse-content").last()
    ).toArray()) {
      const label = $2(genreObj).text().trim();
      const id = label;
  ...
  var parseSearch = ($2) => {
    const results = [];
    for (const item of $2("article.flex.gap-4").toArray()) {
      const id = $2("a", item).attr("href")?.split("/series/")[1]?.split("/")[0] ?? "";
      if (id == "" || typeof id != "string") throw new Error("Id is empty");
      const title = $2("a.link.link-hover", item).first().text().trim() ?? "";
      const image = $2("img", item).attr("src") ?? $2("img", item).attr("data-src") ?? "";
      results.push({
        imageUrl: image,
        title: Application.decodeHTMLEntities(title),
        mangaId: id,
  ...
  var isLastPage = ($2) => {
    return $2('span:contains("View More Results...")').toArray().length == 0;
  };
  ...
  var WeebCentralSettingsForm = class extends import_types3.Form {
    getSections() {
      return [
  ...
  function isEndOfTagSection2(c) {
    return c === CharCodes4.Slash || c === CharCodes4.Gt || isWhitespace4(c);
  }
  ...
  var Tokenizer3 = class {
    ...
    stateInSpecialTag(c) {
      if (this.sequenceIndex === this.currentSequence.length) {
        if (c === CharCodes4.Gt || isWhitespace4(c)) {
          const endOfText = this.index - this.currentSequence.length;
          if (this.sectionStart < endOfText) {
            const actualIndex = this.index;
            this.index = endOfText;
    ...
    stateBeforeTagName(c) {
      if (c === CharCodes4.ExclamationMark) {
        ...
        if (this.xmlMode) {
          this.state = State3.InTagName;
        } else if (lower === Sequences2.ScriptEnd[2]) {
          this.state = State3.BeforeSpecialS;
        } else if (lower === Sequences2.TitleEnd[2] || lower === Sequences2.XmpEnd[2]) {
    ...
    stateInAttributeName(c) {
      if (c === CharCodes4.Eq || isEndOfTagSection2(c)) {
        this.cbs.onattribname(this.sectionStart, this.index);
        this.sectionStart = this.index;
    ...
    stateAfterAttributeName(c) {
      if (c === CharCodes4.Eq) {
        ...
        this.cbs.onattribend(QuoteType2.NoValue, this.sectionStart);
        this.sectionStart = -1;
        this.state = State3.BeforeAttributeName;
        this.stateBeforeAttributeName(c);
      } else if (!isWhitespace4(c)) {
    ...
    handleInAttributeValue(c, quote) {
      if (c === quote || !this.decodeEntities && this.fastForwardTo(quote)) {
        this.cbs.onattribdata(this.sectionStart, this.index);
        this.sectionStart = -1;
        this.cbs.onattribend(quote === CharCodes4.DoubleQuote ? QuoteType2.Double : QuoteType2.Single, this.index + 1);
        this.state = State3.BeforeAttributeName;
      } else if (this.decodeEntities && c === CharCodes4.Amp) {
        this.startEntity();
      }
    ...
    stateInAttributeValueNoQuotes(c) {
      if (isWhitespace4(c) || c === CharCodes4.Gt) {
        this.cbs.onattribdata(this.sectionStart, this.index);
        this.sectionStart = -1;
    ...
    stateBeforeDeclaration(c) {
      if (c === CharCodes4.OpeningSquareBracket) {
        ...
        this.state = c === CharCodes4.Dash ? State3.BeforeComment : State3.InDeclaration;
      }
    }
    stateInDeclaration(c) {
      if (c === CharCodes4.Gt || this.fastForwardTo(CharCodes4.Gt)) {
    ...
    stateBeforeSpecialT(c) {
      const lower = c | 32;
      switch (lower) {
        case Sequences2.TitleEnd[3]: {
          this.startSpecial(Sequences2.TitleEnd, 4);
          break;
        }
        case Sequences2.TextareaEnd[3]: {
          this.startSpecial(Sequences2.TextareaEnd, 4);
          break;
        }
        case Sequences2.XmpEnd[3]: {
          this.startSpecial(Sequences2.XmpEnd, 4);
          break;
        }
        default: {
          this.state = State3.InTagName;
    ...
    startEntity() {
      ...
      this.entityStart = this.index;
      this.entityDecoder.startEntity(this.xmlMode ? DecodingMode2.Strict : this.baseState === State3.Text || this.baseState === State3.InSpecialTag ? DecodingMode2.Legacy : DecodingMode2.Attribute);
    }
    stateInEntity() {
      const length = this.entityDecoder.write(this.buffer, this.index - this.offset);
      if (length >= 0) {
  ...
  var Parser3 = class {
    constructor(cbs, options = {}) {
      ...
      this.lowerCaseAttributeNames = (_b = options.lowerCaseAttributeNames) !== null && _b !== void 0 ? _b : this.htmlMode;
      this.recognizeSelfClosing = (_c = options.recognizeSelfClosing) !== null && _c !== void 0 ? _c : !this.htmlMode;
      this.tokenizer = new ((_d = options.Tokenizer) !== null && _d !== void 0 ? _d : Tokenizer3)(this.options, this);
      this.foreignContext = [!this.htmlMode];
      (_f = (_e = this.cbs).onparserinit) === null || _f === void 0 ? void 0 : _f.call(_e, this);
    }
    // Tokenizer event handlers
    ...
    ontextentity(cp, endIndex) {
      var _a3, _b;
      this.endIndex = endIndex - 1;
      (_b = (_a3 = this.cbs).ontext) === null || _b === void 0 ? void 0 : _b.call(_a3, fromCodePoint2(cp));
      this.startIndex = endIndex;
    }
    /**
     * Checks if the current tag is a void element. Override this if you want
    ...
    emitOpenTag(name) {
      ...
      this.openTagStart = this.startIndex;
      this.tagname = name;
      const impliesClose = this.htmlMode && openImpliesClose2.get(name);
      if (impliesClose) {
        while (this.stack.length > 0 && impliesClose.has(this.stack[0])) {
          const element = this.stack.shift();
          (_b = (_a3 = this.cbs).onclosetag) === null || _b === void 0 ? void 0 : _b.call(_a3, element, true);
    ...
    onclosetag(start, endIndex) {
      ...
      if (this.htmlMode && (foreignContextElements2.has(name) || htmlIntegrationElements2.has(name))) {
        this.foreignContext.shift();
      }
      if (!this.isVoidElement(name)) {
        const pos = this.stack.indexOf(name);
    ...
    onattribend(quote, endIndex) {
      var _a3, _b;
      this.endIndex = endIndex;
      (_b = (_a3 = this.cbs).onattribute) === null || _b === void 0 ? void 0 : _b.call(_a3, this.attribname, this.attribvalue, quote === QuoteType2.Double ? '"' : quote === QuoteType2.Single ? "'" : quote === QuoteType2.NoValue ? void 0 : null);
      if (this.attribs && !Object.prototype.hasOwnProperty.call(this.attribs, this.attribname)) {
        this.attribs[this.attribname] = this.attribvalue;
      }
      this.attribvalue = "";
    ...
    getInstructionName(value) {
      const index2 = value.search(reNameEnd2);
      let name = index2 < 0 ? value : value.substr(0, index2);
      if (this.lowerCaseTagNames) {
        name = name.toLowerCase();
      }
  ...
  // src/WeebCentral/main.ts
  var WEEBCENTRAL_DOMAIN = "https://weebcentral.com";
  var WeebCentralInterceptor = class extends import_types4.PaperbackInterceptor {
    async interceptRequest(request) {
      request.headers = {
        ...request.headers ?? {},
        ...{
  ...
  var WeebCentralExtension = class {
    constructor() {
      ...
      this.mainRequestInterceptor = new WeebCentralInterceptor("main");
    ...
    async getChapterDetails(chapter) {
      ...
      const $2 = await this.fetchCheerio(request);
      let chapters = parseChapterDetails($2, mangaId, chapterId);
    ...
    async getDiscoverSectionItems(section, metadata) {
      ...
      const $2 = await this.fetchCheerio(request);
      const manga = parseViewMore($2, section.id);
      return {
        items: manga,
        metadata
      };
    ...
    async getSearchResults(query, metadata) {
      ...
      const $2 = await this.fetchCheerio(request);
      const results = parseSearch($2);
      metadata = isLastPage($2) ? void 0 : { offset: offset + LIMIT };
      return {
        items: results,
        metadata
      };
  ...
  var WeebCentral = new WeebCentralExtension();
