...
export class Parser {
	async parseMangaDetails(
		...
		const title: string = decodeHTMLEntity(
			$("div.seriestuhead > h1").children().remove().end().text().trim()
		);
		const alternateTitles: string[] = decodeHTMLEntity(
		...
		const description: string = decodeHTMLEntity(
			$("div.entry-content.entry-content-single").first().text()
		)
			.replace("Show more", "")
			.trim();

		const image: string = encodeURI(
			await this.getImageSrc($("div.thumb > img").first(), source)
		);

		const rating: number =
			parseFloat(
				decodeHTMLEntity(
					$("div.rating.bixbox > div > div.num").first().text()
				)
			) * 10;
		const parsedStatus: string = $("tr:nth-child(2) > td:nth-child(2)")
			.first()
			.text()
			.trim();
		...
		const genres: Tag[] = [];
		for (const obj of $("div.seriestucontr > div > a").toArray()) {
	...
	parseChapterList($: CheerioAPI, mangaId: string, source: any): Chapter[] {
		...
		for (const obj of $("#chapterlist > ul > li").toArray()) {
	...
	async parseSearchResults($: CheerioAPI, source: any): Promise<any[]> {
		...
		for (const obj of $(source.searchMangaSelector).toArray()) {
			...
			const subtitle: string = $("span.font-meta.chapter", obj)
				.text()
				.trim();
