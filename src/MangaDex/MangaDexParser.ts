...
export const parseMangaList = async (
	...
	for (const manga of object) {
		...
		const image = coverFileName
			? `${COVER_BASE_URL}/${mangaId}/${coverFileName}${MDImageQuality.getEnding(
					thumbnailSelector()
			  )}`
...
export const parseMangaDetails = (
	...
	if (coverFileName) {
		image = `${COVER_BASE_URL}/${mangaId}/${coverFileName}${MDImageQuality.getEnding(
			getMangaThumbnail()
		)}`;
	}

	const rating = ratingJson.statistics
